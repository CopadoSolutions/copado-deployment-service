...
public class GitServiceImplTest {
    ...
    @Test(expected = CopadoException.class)
    public void fetch_withInvalidBranch() throws CopadoException {
    ...
    private void assertOutdatedRepositoryIncludeChangesAfterFetch(GitSession oldSession) throws CopadoException {
    ...
    private GitSession givenTwoLocalRepositoriesWithSameRemote() throws CopadoException {
        // To do not break the test reference repository, we are going to copy it before pushing.
        GitTestFactory.setUpGitWithNewCopyOfRemote(testFolder);
        session = git.cloneRepo(currentBaseGitDir(), buildCorrectCredentials(currentRemoteDirectory()));
